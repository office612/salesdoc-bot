...
async def add_payment(data: dict) -> int:
    ...
    today = datetime.now(tz2).strftime("%d.%m.%Y")
    ...
    row = [
        today,
        data.get("client", data.get("company", "")),
        data.get("category_label", data.get("category_raw", data.get("category", ""))),
        data.get("license_type", data.get("license_type_raw", "")),
        data.get("qty", data.get("license_qty", "")),
        data.get("manager", ""),
        data.get("period", data.get("tariff", "")),
        data.get("price", ""),
    ...
    # Столбцы N-S пустые (13-18)
    while len(row) < 19:
        row.append("")

    # Новые столбцы T-W (только для апрельских листов и позже)
    start_m = data.get("start_month", "")
    row.append(MONTH_SHEETS.get(int(start_m), "") if start_m else "")  # T

    row.append(data.get("activation_date", ""))  # U

    row.append(data.get("act_price", "") or "")  # V

    cat = data.get("category", "")
    if cat in ("nov_vnedrenie", "nov_integr", "usluga"):
        row.append("Не выполнено")  # W
    else:
        row.append("")  # W
