...
# ID корневой папки на Google Drive (env)
DRIVE_ROOT_FOLDER_ID = os.getenv("DRIVE_RECEIPTS_FOLDER_ID", "")


def _get_drive_service():
    """Создаём Drive API клиент из сервисного аккаунта."""
...
def _find_or_create_folder(service, name: str, parent_id: str) -> str:
    """Ищем папку по имени внутри parent, если нет — создаём."""
    query = (
        f"name = '{name}' "
...
async def upload_receipt(
    ...
    """Загружает файл на Drive в папку месяца, возвращает ссылку."""
